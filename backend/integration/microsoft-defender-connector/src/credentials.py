...
This connector never reads a client_secret from local config or the
environment in production; it only ever reads `credentials_ref` (a
Vault path) from the integration record and resolves it here.
...
class VaultCredentialProvider:
    """Resolves connector credentials from Vault via `credentials_ref`."""

    def __init__(self, vault_client: VaultClient, credentials_ref: str):
        self._vault = vault_client
        self._credentials_ref = credentials_ref
        self._cached: DefenderCredentials | None = None

    async def get_credentials(self, *, force_refresh: bool = False) -> DefenderCredentials:
        ...
        if not client_secret:
            raise VaultAccessError(
                f"Vault secret at '{self._credentials_ref}' is missing 'client_secret'"
            )

        self._cached = DefenderCredentials(
    ...
    def invalidate(self) -> None:
        ...
        used after a rotation event or an auth failure."""
        self._cached = None
