...
from connector_sdk.models import Asset
...
def normalize_splunk_event_to_asset(event: Dict[str, Any], connector_name: str) -> Asset:
    asset_id = str(event.get("_cd") or event.get("id") or event.get("_serial") or hash(str(event)))
    ...
    event_name = event.get("_raw") or event.get("source") or f"Splunk Event {asset_id}"
    ...
    return Asset(
        ...
        tags={
            "index": str(event.get("index", "unknown")),
