...
from sentinelone_connector.base import Asset
from sentinelone_connector.push import BetaPlatformPusher


def make_assets(n):
    return [Asset(external_id=f"a{i}", source="sentinelone", asset_type="endpoint", name=f"h{i}") for i in range(n)]


def test_single_batch_success():
    s = MagicMock()
    s.post.return_value = MagicMock(status_code=200)
    r = BetaPlatformPusher("tok", session=s).push("sentinelone", make_assets(5))
...
def test_batches_large_lists():
    ...
    r = BetaPlatformPusher("tok", session=s).push("sentinelone", make_assets(450), batch_size=200)
...
def test_failed_batch_recorded():
    ...
    r = BetaPlatformPusher("tok", session=s).push("sentinelone", make_assets(3))
