...
_REPORTS_SERVICE_NAME = "admin"
_REPORTS_SERVICE_VERSION = "reports_v1"
...
class GoogleWorkspaceAuthenticator:
    ...
    def get_reports_service(self) -> Resource:
        """
        Return a cached Admin SDK Reports API client (for audit log
        activities), authenticating if needed. Uses the same delegated
        credentials as the Directory API — both are part of the same
        Admin SDK, just different sub-APIs.
        """
        if self._credentials is None or not self._credentials.valid:
            self.authenticate()
        if self._reports_service is None:
            self._reports_service = build(
                _REPORTS_SERVICE_NAME,
                _REPORTS_SERVICE_VERSION,
                credentials=self._credentials,
                cache_discovery=False,
            )
