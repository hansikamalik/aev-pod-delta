...
Week 2 deliverable (Sep 11 - Sep 17), Pod Delta M4.
...
from .credentials import VaultClientProtocol, load_github_credentials
...
class GitHubConnector(Connector):
    ...
    def __init__(self, vault: VaultClientProtocol, org_id: str, beta_api_token: str):
        ...
        self._auth_config = load_github_credentials(vault, org_id)
        self._authenticator = GitHubAuthenticator(self._auth_config)
        self._discovery = GitHubDiscovery(self._authenticator)
        self._pusher = BetaPlatformPusher(api_token=beta_api_token)

    @property
    def warnings(self) -> List[str]:
        """Non-fatal issues from the last ingest (e.g. an alert type skipped)."""
        return self._discovery.warnings

    def authenticate(self) -> bool:
        ok = self._authenticator.validate()
        if ok:
            logger.info("github_connector.auth.ok org_id=%s", self.org_id)
        else:
            logger.error("github_connector.auth.failed org_id=%s", self.org_id)
        return ok

    def discover(self) -> List[Dict[str, Any]]:
        repos = self._discovery.discover()
        logger.info("github_connector.discover.count=%d org_id=%s", len(repos), self.org_id)
        return repos

    def ingest(self) -> List[Dict[str, Any]]:
        enriched = self._discovery.ingest(self.discover())
        logger.info("github_connector.ingest.count=%d org_id=%s", len(enriched), self.org_id)
        return enriched
    ...
    def normalize(self, raw_records: List[Dict[str, Any]]) -> List[Asset]:
        ...
        logger.info(
            "github_connector.normalize assets=%d findings=%d org_id=%s",
    ...
    def push(self, assets: List[Asset]) -> SyncResult:
        ...
        if result.success:
            logger.info("github_connector.push.ok assets=%d org_id=%s", result.assets_count, self.org_id)
        else:
            logger.error("github_connector.push.errors=%s org_id=%s", result.errors, self.org_id)
        return result
